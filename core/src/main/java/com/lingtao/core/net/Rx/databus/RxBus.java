package com.lingtao.core.net.Rx.databus;
...
public class RxBus {
     ...
     * @param data
     */
    private void callMethodByAnnotion(Object subscriber, Object data) {
        //1.得到presenter中写的所有的方法
        Method[] methodArray = subscriber.getClass().getDeclaredMethods();
            ...
            try {
                if (methodArray[i].getAnnotation(RegisterRxBus.class) != null) {
                    ...
                    if (data.getClass().getName().equals(paramType.getName())) {
                        //执行
                        methodArray[i].invoke(subscriber, new Object[]{data});
                    }
                }
            }catch (Exception e){
                e.printStackTrace();
